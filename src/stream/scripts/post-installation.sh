...
# For rowhammer benchmarks
apt-get install -y g++
apt-get install -y gcc
apt-get -qq --yes --force-yes install cmake
apt-get install -y make

apt-get install -y libboost-all-dev ndctl

echo "Building disaggregated stuff before we loose root access!"

# build gem5 util
git clone https://github.com/gem5/gem5.git gem5-util
cd gem5-util/include
export M5OPS_HEADER_PATH=`pwd`
cd ../util/m5
...
cd /home/gem5
git clone https://github.com/kaustav-goswami/simple-vectorizable-microbenchmarks
cd simple-vectorizable-microbenchmarks/
git submodule update --init --recursive
cd stream-syscalls
make -f makefiles/Makefile-hw -j8 
cd ../..
cd simple-vectorizable-microbenchmarks/stream-jemalloc
make -f makefiles/Makefile-hw -j8 
cd ../..
cd simple-vectorizable-microbenchmarks/stream-tcmalloc
make -f makefiles/Makefile-hw -j8 
cd ../..

echo "all stream artifacts built!"
