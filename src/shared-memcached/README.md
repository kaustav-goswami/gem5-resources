---
title: Shared GAP Benchmark Suite (GAPBS) tests with Ubuntu 24.04
tags:
    - x86
    - fullsystem
shortdoc: >
...
See https://github.com/darchr/shared-gapbs/blob/shared-disaggregated-v3/README-SDM.md on how the benchmark is structured.

The artifacts are compiled in the `shared-gapbs` directory.

A type 12 X86 memory range is needed to create a pmem/dax memory region.
The `dev/dax0.0` device needs to be mounted at startup.
This can be done by:
```sh
(guest) echo 12345 | sudo -S ndctl create-namespace -f -enamespace0.0 -m devdax
```
